...
operator and the agency—defaults in the chart. The AMF and SMF take the
standard's own behaviour there, because the agreement is between you and
the agency and this project is not party to it. The UPF does not: the
chart refuses bulk deactivation on it, since its X1 peer is the
deployment's own SMF and no triggering function here sends a bulk
message. A normal single-UPF deployment therefore needs nothing beyond
the block above; multi-UPF or renamed deployments override the trigger
list in the chart values (``config.smf.li.upfTriggers``), and a
deployment whose agreement differs sets the bulk-operation keys described
in ``li/README.md``.
